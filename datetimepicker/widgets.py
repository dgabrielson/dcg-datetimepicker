...
FORMAT_MAP = {'d': r'%d', 'm': r'%m', 'Y': r'%Y', 'H': r'%H', 'i': r'%M'}


def _py_datetime_format_to_js(format_string):
    return (format_string is not None and reduce(
        lambda format_string, args: format_string.replace(*reversed(args)),
        FORMAT_MAP.items(), format_string) or None)
...
class DateTimePicker(DateTimeInput):
    ...
    def render(self,
        ...
        if value != '':
            ...
            input_attrs.update({'value': force_text(self.format_value(value))})

        self.div_attrs.update({
            'id':
            '{prefix}_{field_id}'.format(
                prefix=prefix,
        ...
        if self.use_script_tag:
            js_options = json.dumps(
                dict(
                    format=_py_datetime_format_to_js(
                        self.options.get('format')),
                    **{
                        key: val
                        for key, val in self.options.items() if key != 'format'
                    }))
            js = render_to_string(
